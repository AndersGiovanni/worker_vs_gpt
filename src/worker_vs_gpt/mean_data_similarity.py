...
def main(cfg: SimilarityConfig) -> None:
    ...
    augmented_dataset["augment_text_length"] = augmented_dataset[text].apply(lambda x: len(tokenizer.encode(x)))
    train_dataset["train_text_length"] = train_dataset[text].apply(lambda x: len(tokenizer.encode(x)))
    ...
    similarity_dict = {
        ...
        "augment_text_length": {
            "mean": augmented_dataset["augment_text_length"].mean(),
            "std": augmented_dataset["augment_text_length"].std(),
        },
        "train_text_length": {
            "mean": train_dataset["train_text_length"].mean(),
            "std": train_dataset["train_text_length"].std(),
        },
    }
