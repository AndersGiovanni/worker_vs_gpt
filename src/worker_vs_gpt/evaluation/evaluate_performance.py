...
for ax_idx, folder_path in enumerate(folders_to_evaluate):
    files_to_evaluate: List[Path] = [
        file_path for file_path in folder_path.iterdir() if file_path.suffix == ".json"
    ]
    ...
    labels = (
        metric_df["src label"].values.tolist() + metric_df["aug label"].values.tolist()
    )
    ...
    model_metrics.to_csv(f"src/worker_vs_gpt/evaluation/assets/{model_type}.csv")
